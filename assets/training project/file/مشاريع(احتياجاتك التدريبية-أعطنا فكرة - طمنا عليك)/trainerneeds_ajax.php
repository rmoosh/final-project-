...
function trainerneeds()
{
	...
	console.log($('#trainerneedsform').serializeArray());
	 //alert("academy_Id: " + $('#academy_Id').val() );
...
if (empty($_POST['Qtans']))
{
		 $url1 ="trainerneeds.php";

		$msg["txt"] = "يجب الاجابة على جميع الاسئلة";
		//die(json_encode($msg));
		//exit();
				echo "<script language='javascript'><!--\n";
			    echo 'alert("' . $msg. '")';
			    echo "//--></script>\n\n"; 
			  	die('<meta http-equiv="refresh" content="0;URL='.$url1.'">');  
...
//**********************************************
  $ans="النتيجة";
//$trainerneed="المستوى إيجابي";
 $QNumber=count(@$_POST['QsnID']);
 
 $need=" تحتاج الى دورة في :";

 $k=0;
 for ($i=0; $i<count(@$_POST['QsnID']) ;$i++) 
{

	 
if (!(empty($_POST['Qtans'][$i])))
{
$QsnID=@$_POST['QsnID'][$i];


//correct
$QsnName= @$_POST["QsnName"][$i];
		 if (@$_POST['Qtans'][$i] == "False") 
		{
			 
			 $k+=1;
			//*********************
			
			$stmt = $db->prepare("select * from trainerneeds where QsnID='$QsnID' ") ;
			...
			if($row)
			{
		     	...
		     	$trainerneed= $row["trainerneed"];
		     	if($k==1)
				{
					echo("<p>");
					 echo"<p style='font-size: medium'>" .$need ;
				}
				...
				echo("<p>");
				echo("<p>");
	           // echo"<td style='font-size: medium'>" .$k. ':)' .$QsnName;
	           // echo("</td>");
	            //echo("</tr>");
		        //-------------------------------------	
		        echo("<p>");
		        echo("<tr>");
		        echo"<td style='font-size: medium'>" .$k. ':)'  .$trainerneed;
		        echo("</td>");
		        echo("</tr>");
		        echo("</table>");
		...
		else
		{
				...
				echo("<p>");
				echo("<p>");
	            echo"<td style='font-size: medium'>" .$i. ':)' .$QsnName;
	            echo("</td>");
	            echo("</tr>");
		        //-------------------------------------	
		        echo("<p>");
	       		 echo("<tr>");
		        echo"<td style='font-size: medium'>" .$ans. ':)' .$trainerneed;
		        echo("</td>");
		        echo("</tr>");
		        echo("</table>");
